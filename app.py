...
def init_db():
    ...
    if conn:
        ...
        print("⚠️ Running on fallback memory.")
...
def upload_file():
    ...
    if file and file.filename != '' and allowed_file(file.filename):
        ...
        try:
            ...
            flash(f"🤝 Thank you! Material for {unit} ({year}) uploaded.")
        except Exception as e:
...
def delete_file(filename):
    ...
    if conn:
        ...
        if filename in LOCAL_FALLBACK["cloudinary_urls"]:
            del LOCAL_FALLBACK["cloudinary_urls"][filename]
            flash(f"File '{filename}' removed from memory.")
        else:
            flash("Error: File not found.")
    return redirect('/')
